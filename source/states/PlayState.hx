...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		...
		['TLC', 0.4], // From 20% to 39%
		['UFE', 0.5], // From 40% to 49%
		['Almost UFE', 0.6], // From 50% to 59%
		['Barely UFE', 0.69], // From 60% to 68%
		['Nice!', 0.7], // 69%
		['Like, good!', 0.8], // From 70% to 79%
		['Like like like great!', 0.9], // From 80% to 89%
		['YOYLECAKE!!', 1], // From 90% to 99%
		['YOU GET THE PRIZE!!!!', 1] // The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	override public function create()
	{
		...
		if (isStoryMode)
			detailsText = "Season " + WeekData.getCurrentWeek().weekName;
		else
		...
		botplayTxt.visible = cpuControlled && !ClientPrefs.data.iSuck;
	...
	function doDeathCheck(?skipHealthCheck:Bool = false)
	{
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if (ret != LuaUtils.Function_Stop)
			{
				...
				if (SONG.stage == "limo")
				{
					Achievements.unlock("say_goodbye_2");
				}
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		{
			songScore += score;
			if (!note.ratingDisabled && ClientPrefs.data.iSuck)
			{
				songHits++;
				totalPlayed++;
